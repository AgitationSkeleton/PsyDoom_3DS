...
void flushToDisk() noexcept {
    ...
    // Only go looking at the config files if one is actually waiting to be written. 'writeAllConfigFiles' checks the
    // same flags, but not before working out the folder to write into, which is itself a trip to the card.
    const bool bAnyConfigDirty = (
        Config::gbNeedSave_Audio ||
        Config::gbNeedSave_Controls ||
        Config::gbNeedSave_Game ||
        Config::gbNeedSave_Graphics ||
        Config::gbNeedSave_Input ||
        Config::gbNeedSave_Multiplayer
    );
...
//------------------------------------------------------------------------------------------------------------------------------------------
void save() noexcept {
    // Build the file in memory rather than writing straight out, so it can be compared against what was written last
    std::string prefsText;

    const auto appendLine = [&](const char* const format, auto... args) noexcept {
        char line[256];
    ...
    appendLine("; WARNING: this file is auto-generated by PsyDoom, it may be overwritten at any time!\n");
    appendLine("soundVol = %d\n", static_cast<int>(gSoundVol));
    appendLine("musicVol = %d\n", static_cast<int>(gMusicVol));
    appendLine("lastPassword_Doom = %s\n", getPasswordCString(gLastPassword_Doom).chars);
    appendLine("lastPassword_FinalDoom = %s\n", getPasswordCString(gLastPassword_FDoom).chars);
    appendLine("lastPassword_GecMe = %s\n", getPasswordCString(gLastPassword_GecMe).chars);
    appendLine("turnSpeedPercentMultiplier = %d\n", static_cast<int>(gTurnSpeedMult100));
    appendLine("alwaysRun = %d\n", (int) gbAlwaysRun);
#if PSYDOOM_3DS
    appendLine("detailMode = %d\n", (int) gDetailMode);
    appendLine("fullWidthVideo = %d\n", (int) gbFullWidthVideo);
    appendLine("statusBarPos = %d\n", (int) gStatusBarPos);
    appendLine("controlScheme = %d\n", (int) gControlScheme);
#else
    appendLine("uncapFramerate = %d\n", (int) gbUncapFramerate);
#endif
    appendLine("statDisplayMode = %d\n", (int) gStatDisplayMode);
#if !PSYDOOM_3DS
    appendLine("startupWithVulkanRenderer = %d\n", (int) Video::isUsingVulkanRenderPath());
#endif

    // Nothing to do if the card already holds exactly this. Backing out of a menu calls this whether or not anything
    // was changed, and a write is slow enough on a console to be felt as a pause.
    static std::string sLastWritten;

    if (prefsText == sLastWritten)
        return;
    ...
    std::fwrite(prefsText.data(), 1, prefsText.size(), pFile);
    std::fflush(pFile);
    std::fclose(pFile);
    sLastWritten = prefsText;
}
